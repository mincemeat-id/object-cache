...
The strict compatibility smoke command loads the pinned WooCommerce 11.0.0,
Yoast SEO 28.2, Easy Digital Downloads 3.6.9, and Query Monitor 4.0.7 entry points,
runs their real activation/schema installers, and fails for missing or mismatched
plugins, unexpected PHP diagnostics, or any post-install WordPress database error. Its
...
Create controlled RC3/current performance evidence against Redis 8:

```bash
composer benchmark:controlled -- 127.0.0.1 6383
```

The current harness runs the immutable `0.1.0-rc3` runtime and two current
...
the relative change exceeds 25%; an RC4 regression against RC3 uses the same
dual threshold, while command, round-trip, and connection increases fail
deterministically. A companion `tools/benchmark-memory.php` records a non-gating
request-memory soak (peak and allocated-byte deltas) as `rc3-memory.json` /
`rc4-memory.json` so before/after memory can be reported alongside time. CI
uploads all reports and comparisons. Connection targets are intentionally
omitted. The raw Redis EVAL/EVALSHA controls remain visible but informational
because they do not execute the compared runtime.
...
The lifecycle suite builds packages from the immutable `0.1.0-rc3` tag and the
...
containing the RC3 tag is required.
